...
2022-07-11          B. Klein                            Renamed _demo_ to _DCR_CLEANROOMPOC_ to support upgrades.
...
create or replace database dcr_DCR_CLEANROOMPOC_XOB00824 from share XOB00824.dcr_DCR_CLEANROOMPOC_requests_MFB86782;
create or replace stream dcr_DCR_CLEANROOMPOC_provider_db.admin.request_stream_XOB00824 on table dcr_DCR_CLEANROOMPOC_XOB00824.MFB86782_schema.requests append_only = true show_initial_rows = true
comment='{"origin":"sf_ps_wls","name":"dcr","version":{"major":5, "minor":5},"attributes":{"component":"dcr",“role”:“provider”}}';
delete from dcr_DCR_CLEANROOMPOC_provider_db.admin.request_log;

// see request status and streams

// see requests
select * from dcr_DCR_CLEANROOMPOC_XOB00824.MFB86782_schema.requests;
// see request stream
select * from dcr_DCR_CLEANROOMPOC_provider_db.admin.request_stream_XOB00824;
// see if the stream has anything in it
select SYSTEM$STREAM_HAS_DATA('dcr_DCR_CLEANROOMPOC_provider_db.admin.request_stream_XOB00824');
...
create or replace procedure dcr_DCR_CLEANROOMPOC_provider_db.admin.process_requests(party_account string)
returns string
language javascript
comment='{"origin":"sf_ps_wls","name":"dcr","version":{"major":5, "minor":5},"attributes":{"component":"dcr",“role”:“provider”}}'
execute as owner
...
//pull request parameters from request stream
 ...
 var request_tbl_sql =`create or replace table dcr_DCR_CLEANROOMPOC_provider_db.admin.request_tmp as select REQUEST:REQUEST_ID as REQUEST_ID,
...
from dcr_DCR_CLEANROOMPOC_provider_db.admin.request_stream_`+PARTY_ACCOUNT+` `;

var request_tbl_sql_exec= snowflake.execute({sqlText: request_tbl_sql});
request_tbl_sql_exec.next();

  // check data in stream
  var result = snowflake.execute({ sqlText: `select count(1) from dcr_DCR_CLEANROOMPOC_provider_db.admin.request_tmp` });
  result.next();
 ...
 // get template available fields
  var result = snowflake.execute({ sqlText: `select lower(dimensions) from dcr_DCR_CLEANROOMPOC_provider_db.templates.dcr_templates where party_account=(select any_value(requester_account) from dcr_DCR_CLEANROOMPOC_provider_db.admin.request_tmp) and template_name = (select any_value(query_template) from dcr_DCR_CLEANROOMPOC_provider_db.admin.request_tmp)` });
  result.next();
  var dimensions = result.getColumnValue(1);

// get request_id
  var result = snowflake.execute({ sqlText: `select REQUEST_ID,select_fields from dcr_DCR_CLEANROOMPOC_provider_db.admin.request_tmp` });
  result.next();
  var request_id = result.getColumnValue(1);
  var select_field = result.getColumnValue(2);

//Validation based on Input
    var unapproved_entry = ` insert into dcr_DCR_CLEANROOMPOC_provider_db.admin.request_log (party_account, request_id, request_ts, request, app_instance_id,query_hash,    template_name, epsilon, sensitivity, processed_ts, approved,error)
        with request as (
        ...
        ,req.request
        ,req.REQUEST_PARAMS
        from dcr_DCR_CLEANROOMPOC_provider_db.admin.request_tmp req
          )
  ...
  //1.TimeStamp check that request is not too old or from the future
     ...
     var result = snowflake.execute({ sqlText: `select request_ts from dcr_DCR_CLEANROOMPOC_provider_db.admin.request_tmp` });
     result.next();
 ...
 // verify list of available values
    if (select_field == null) {
      ...
      // verify list of available values
        ...
        from dcr_DCR_CLEANROOMPOC_provider_db.admin.request_tmp)
        ,tem_dim as
...
var sql_text = `
insert into dcr_DCR_CLEANROOMPOC_provider_db.admin.request_log (party_account, request_id, request_ts, request, app_instance_id,  query_hash, template_name, epsilon, sensitivity, processed_ts, approved,error)
with request as (
 select req.REQUEST_ID
 ,req.query_template query_template
 ,req.requester_account requester_account
...
proposed_query as (
 select dcr_DCR_CLEANROOMPOC_provider_db.templates.get_sql_jinja(
     (select any_value(template) from dcr_DCR_CLEANROOMPOC_provider_db.templates.dcr_templates where template_name = rp.query_template
...
validate_sql as (
  ...
  dcr_DCR_CLEANROOMPOC_provider_db.templates.get_sql_jinja(template, request_params) as valid_sql,
  ...
  from request ,proposed_query pq, dcr_DCR_CLEANROOMPOC_provider_db.templates.dcr_templates t
...
snowflake.execute({ sqlText: sql_text });
  ...
  // get proposed query
    var result = snowflake.execute({ sqlText: `select query_hash from dcr_DCR_CLEANROOMPOC_provider_db.admin.request_log where REQUEST_ID ='`+request_id+`'; ` });
    result.next();
...
// process approvals manually one-time, replace XOB00824 with the consumer account name

call dcr_DCR_CLEANROOMPOC_provider_db.admin.process_requests('XOB00824');

////////
/// ENABLE EACH XOB00824
/// Replace all instances of XOB00824 below with the name of the consumer account
////////

// set up a fleet of tasks to automatically process consumer requests

CREATE OR REPLACE TASK dcr_DCR_CLEANROOMPOC_provider_db.admin.process_requests_XOB00824_1
  SCHEDULE = '1 minute'  WAREHOUSE = 'app_wh'
WHEN  SYSTEM$STREAM_HAS_DATA('dcr_DCR_CLEANROOMPOC_provider_db.admin.request_stream_XOB00824')
AS call dcr_DCR_CLEANROOMPOC_provider_db.admin.process_requests('XOB00824');

CREATE OR REPLACE TASK dcr_DCR_CLEANROOMPOC_provider_db.admin.process_requests_XOB00824_2 SCHEDULE = '1 minute'  WAREHOUSE = 'app_wh' WHEN  SYSTEM$STREAM_HAS_DATA('dcr_DCR_CLEANROOMPOC_provider_db.admin.request_stream_XOB00824') AS call dcr_DCR_CLEANROOMPOC_provider_db.admin.process_requests('XOB00824');
CREATE OR REPLACE TASK dcr_DCR_CLEANROOMPOC_provider_db.admin.process_requests_XOB00824_3 SCHEDULE = '1 minute'  WAREHOUSE = 'app_wh' WHEN  SYSTEM$STREAM_HAS_DATA('dcr_DCR_CLEANROOMPOC_provider_db.admin.request_stream_XOB00824') AS call dcr_DCR_CLEANROOMPOC_provider_db.admin.process_requests('XOB00824');
CREATE OR REPLACE TASK dcr_DCR_CLEANROOMPOC_provider_db.admin.process_requests_XOB00824_4 SCHEDULE = '1 minute'  WAREHOUSE = 'app_wh' WHEN  SYSTEM$STREAM_HAS_DATA('dcr_DCR_CLEANROOMPOC_provider_db.admin.request_stream_XOB00824') AS call dcr_DCR_CLEANROOMPOC_provider_db.admin.process_requests('XOB00824');
CREATE OR REPLACE TASK dcr_DCR_CLEANROOMPOC_provider_db.admin.process_requests_XOB00824_5 SCHEDULE = '1 minute'  WAREHOUSE = 'app_wh' WHEN  SYSTEM$STREAM_HAS_DATA('dcr_DCR_CLEANROOMPOC_provider_db.admin.request_stream_XOB00824') AS call dcr_DCR_CLEANROOMPOC_provider_db.admin.process_requests('XOB00824');
CREATE OR REPLACE TASK dcr_DCR_CLEANROOMPOC_provider_db.admin.process_requests_XOB00824_6 SCHEDULE = '1 minute'  WAREHOUSE = 'app_wh' WHEN  SYSTEM$STREAM_HAS_DATA('dcr_DCR_CLEANROOMPOC_provider_db.admin.request_stream_XOB00824') AS call dcr_DCR_CLEANROOMPOC_provider_db.admin.process_requests('XOB00824');

// enable tasks 8 seconds apart to reduce consumer wait time for request approval

ALTER TASK dcr_DCR_CLEANROOMPOC_provider_db.admin.process_requests_XOB00824_1 RESUME;
call system$wait(8);
ALTER TASK dcr_DCR_CLEANROOMPOC_provider_db.admin.process_requests_XOB00824_2 RESUME;
call system$wait(8);
ALTER TASK dcr_DCR_CLEANROOMPOC_provider_db.admin.process_requests_XOB00824_3 RESUME;
call system$wait(8);
ALTER TASK dcr_DCR_CLEANROOMPOC_provider_db.admin.process_requests_XOB00824_4 RESUME;
call system$wait(8);
ALTER TASK dcr_DCR_CLEANROOMPOC_provider_db.admin.process_requests_XOB00824_5 RESUME;
call system$wait(8);
ALTER TASK dcr_DCR_CLEANROOMPOC_provider_db.admin.process_requests_XOB00824_6 RESUME;
show tasks in  dcr_DCR_CLEANROOMPOC_provider_db.admin;

// use these to later pause the clean room request approval tasks, if needed

//ALTER TASK dcr_DCR_CLEANROOMPOC_provider_db.admin.process_requests_XOB00824_1 SUSPEND;
//ALTER TASK dcr_DCR_CLEANROOMPOC_provider_db.admin.process_requests_XOB00824_2 SUSPEND;
//ALTER TASK dcr_DCR_CLEANROOMPOC_provider_db.admin.process_requests_XOB00824_3 SUSPEND;
//ALTER TASK dcr_DCR_CLEANROOMPOC_provider_db.admin.process_requests_XOB00824_4 SUSPEND;
//ALTER TASK dcr_DCR_CLEANROOMPOC_provider_db.admin.process_requests_XOB00824_5 SUSPEND;
//ALTER TASK dcr_DCR_CLEANROOMPOC_provider_db.admin.process_requests_XOB00824_6 SUSPEND;
