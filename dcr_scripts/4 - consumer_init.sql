...
execute immediate $$
declare
  res resultset default (select $4 as name from table(result_scan(last_query_id())) where $5 = 'DCR_DCR_CLEANROOMPOC_CONSUMER');
  c1 cursor for res;
  share_var string;
begin
  open c1;
...
///////
// XOB00824 LOCAL SETUP
///////

create or replace database dcr_DCR_CLEANROOMPOC_consumer;
create or replace schema dcr_DCR_CLEANROOMPOC_consumer.util;

// Python jinja function
create or replace function dcr_DCR_CLEANROOMPOC_consumer.util.get_sql_jinja(template string, parameters variant)
  returns string
  language python
  runtime_version = 3.8
...
create or replace schema dcr_DCR_CLEANROOMPOC_consumer.MFB86782_schema;
create table dcr_DCR_CLEANROOMPOC_consumer.util.instance 
comment='{"origin":"sf_ps_wls","name":"dcr","version":{"major":5, "minor":5},"attributes":{"component":"dcr",“role”:“consumer”}}' 
as select randstr(64,random()) as id;


///////
// XOB00824 INSTALLS THE CLEAN ROOM APP - MFB86782_1
///////

// mount clean room app
create or replace database dcr_DCR_CLEANROOMPOC_app from share MFB86782.dcr_DCR_CLEANROOMPOC_app;

// see templates from provider
select * from dcr_DCR_CLEANROOMPOC_app.cleanroom.templates;


///////
// XOB00824 SETS UP REQUEST TABLES AND SHARES
///////

// create the request tables
create or replace table dcr_DCR_CLEANROOMPOC_consumer.MFB86782_schema.requests (request_id varchar(1000),request variant, signature varchar(1000))
comment='{"origin":"sf_ps_wls","name":"dcr","version":{"major":5, "minor":5},"attributes":{"component":"dcr",“role”:“consumer”}}';
ALTER TABLE dcr_DCR_CLEANROOMPOC_consumer.MFB86782_schema.requests SET CHANGE_TRACKING = TRUE;

// share the request table to the provider
create or replace share dcr_DCR_CLEANROOMPOC_requests_MFB86782;
grant usage on database dcr_DCR_CLEANROOMPOC_consumer to share dcr_DCR_CLEANROOMPOC_requests_MFB86782;
grant usage on schema dcr_DCR_CLEANROOMPOC_consumer.MFB86782_schema to share dcr_DCR_CLEANROOMPOC_requests_MFB86782;
grant select on dcr_DCR_CLEANROOMPOC_consumer.MFB86782_schema.requests to share dcr_DCR_CLEANROOMPOC_requests_MFB86782;
alter share dcr_DCR_CLEANROOMPOC_requests_MFB86782 add accounts = MFB86782;

// test share from app, see app instance unique id - provider1
//select * from dcr_DCR_CLEANROOMPOC_app.cleanroom.instance;
...
create or replace procedure dcr_DCR_CLEANROOMPOC_consumer.MFB86782_schema.request(in_template varchar(1000), in_params varchar(10000), request_id varchar(1000), at_timestamp VARCHAR(30))
    returns variant
    language javascript
    comment='{"origin":"sf_ps_wls","name":"dcr","version":{"major":5, "minor":5},"attributes":{"component":"dcr",“role”:“consumer”}}'
    execute as owner as
    ...
    var result = snowflake.execute({ sqlText:  `select account_name from dcr_DCR_CLEANROOMPOC_app.cleanroom.provider_account ` });
    result.next();
    ...
    settings as
     (
     select object_agg(setting_name, setting_value::variant) as settings from dcr_DCR_CLEANROOMPOC_consumer.local.user_settings
      ),
     app_instance_id as
      (
     select any_value(id) as app_instance_id from dcr_DCR_CLEANROOMPOC_consumer.util.instance
     ),
    ...
    (
     ...
     from query_params, settings, request_params, app_instance_id
             ...
             select dcr_DCR_CLEANROOMPOC_consumer.util.get_sql_jinja(
            (select template from dcr_DCR_CLEANROOMPOC_app.cleanroom.templates where template_name = rp.query_template), qpf.request_params) as proposed_query,
  ...
  // put request JSON into a temporary place so if it is approved we can use it later directly from SQL to avoid JS altering it
   var result = snowflake.execute({ sqlText: `create or replace table dcr_DCR_CLEANROOMPOC_consumer.MFB86782_schema.request_temp as select REQUEST FROM      table(result_scan(last_query_id()));` });

  var signed_request ='';

 //insert the signed request
  var insert_sql =  `insert into dcr_DCR_CLEANROOMPOC_consumer.MFB86782_schema.requests (request_id , request, signature )
  select (select any_value(request:REQUEST_ID) from MFB86782_schema.request_temp),(select any_value(request) from MFB86782_schema.request_temp), '`+signed_request+`' ; `;
  var r_stmt = snowflake.createStatement( { sqlText: insert_sql } );
  var result = r_stmt.execute();

  var result = snowflake.execute({ sqlText:`select any_value(request:REQUEST_ID) from MFB86782_schema.request_temp` });
  result.next();
